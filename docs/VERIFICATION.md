...
| Balanced Secure Event Input and indication | Injected native acquire/yield/reacquire/failure/external-owner/shutdown tests, no-layout accessible overlay checks, typed Dart owner/snapshot/failure/state tests, and optional current/legacy FFI | Verified |
...
| v12 application local-folder Services provider | Native `openTab`/`openWindow`, filesystem directory/file normalization, ordered de-duplication, type/URL/limit/error/disable/shutdown tests, exact encoder and v11 filtering, typed Dart packet/cache/routing/failure/legacy tests, current/legacy FFI | Verified |
